...
ProductionOptionalBoost.VERSION = "0.1.3.0"
...
-- Production menu recipe visualization. Optional boosters are exposed to the
-- stock recipe renderer only for the duration of recipeCell population, so
-- they never become mandatory production.inputs.
local function formatBoostPercent(bonus)
    return g_i18n:formatNumber(math.max(bonus or 0, 0) * 100, 2)
end
...
local function setBoostTextColor(element, state)
    ...
    if state == 2 then
        -- Enough material for at least one fully boosted cycle.
        element:setTextColor(0.305, 0.85, 0.10, 1.0)
    elseif state == 1 then
        -- A partial amount is available; runtime logic will scale the bonus.
...
function ProductionOptionalBoost.productionMenuPopulateCell(
    ...
    -- Restore the real production recipe immediately after stock rendering.
    ...
    -------------------------------------------------------------------------
    -- Input side: decorate the synthetic booster rows generated by stock UI.
    -------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------
    -- Output side: show the maximum additional amount from all inputBoosts
    -- that affect each output. Example:
    --   850 (+192.53) x FORAGE
    -------------------------------------------------------------------------
        ...
        for outputIndex, output in ipairs(production.outputs or {}) do
            ...
            if bonusFactor > ProductionOptionalBoost.EPSILON then
                ...
                if amountElement ~= nil then
                    local bonusAmount = output.amount * bonusFactor
